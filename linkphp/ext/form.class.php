...
class form{
	// 文本
	public static function get_text($name, &$val, $class='') {
		return '<input name="'.$name.'" type="text" value="'.htmlspecialchars($val).'" class="'.$class.'"/>';
	}

	// 多行文本
	public static function get_textarea($name, &$val, $class='') {
		return '<textarea rows="5" name="'.$name.'" class="'.$class.'">'.htmlspecialchars($val).'</textarea>';
	}

	// 密码
	public static function get_password($name, &$val, $class='text') {
		return '<input name="'.$name.'" type="password" value="'.$val.'" class="'.$class.'" />';
	}

	// 数字
	public static function get_number($name, &$val, $class='text') {
