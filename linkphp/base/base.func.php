...
function E($status, $msg, $name = ''){
	exit('{"status":'.$status.', "message":"'.$msg.'", "name":"'.$name.'"}');
}
 ...
 * @return string
 */
function pages($page, $maxpage, $url, $offset = 4, $lang = array('&#171;上页', '下页&#187;')){
	if($maxpage < 2) return '';
	$pnum = $offset*2;
	$ismore = $maxpage > $pnum;
	$s = '';
...
/**
 * 一维数组转字符串
 * $arr 一维数组; $connector 连接符
 */
function arr_to_string ($arr,$connector=','){
	if(!empty($arr)) {
		foreach($arr as $v) {
			$newarr[] = $v;
		}
		return implode($connector,$newarr);
	}
}
...
function GrabImage($url, $filename = ""){
	...
	if ($filename == "") {//如果没有指定新的文件名
		...
		$filename = '/upload/'.date('Y').'/'.date('m').'/'.date('d').'/'.date("YmdHis") . '_' . rand(10000, 99999) . '.' . $ext;//用天月面时分秒来命名新的文件名
 	}
 	ob_start();//打开输出
	readfile($url);//输出图片文件
