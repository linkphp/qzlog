<?php
/*
在模型设计上我思考了很久，最终确定：统一 cache+db 接口，并设计了二级缓存，进一步减轻数据库压力。
开启 cache 时：
...
class model{
	...
	*/
	...
	/**
	 * 读取一条数据
	 * @param string $arg1 主键id
	 * @param int $life 缓存时间（默认不缓存）
	 * @return array
	 */
	public function read($arg1, $life = 0) {
		$arr = (array)$arg1;
		return $this->get($arr,$life);
	}
	...
	public function get($arr, $life = 0) {
		$key = $this->arr2key($arr);
		if(!isset($this->unique[$key])) {
			$this->unique[$key] = $this->cache_db_get($key, $life);
		}
	 ...
	 * @return mixed
	 */
	public function cache_db_get($key, $life = 0) {
		if($this->cache_conf['enable'] && $life > 0) {
	 ...
	 * @param int $life 缓存时间 （默认为不缓存）
	 * @return array
	 */
	public function cache_db_multi_get($keys,$life = 0) {
		if($this->cache_conf['enable'] && $life > 0) {
			$data = $this->cache->multi_get($keys);
			if(empty($data)) {
				$data = $this->db->multi_get($keys);
				foreach((array)$data as $k=>$v) {
					$this->cache->set($k, $v, $life);
				}
			}else{
				foreach($data as $k=>&$v) {
	...
	public function cache_db_find_fetch($table, $pri, $where = array(), $order = array(), $start = 0, $limit = 0, $life = 0) {
		...
		if($this->db_conf['type'] == 'mongodb') {
			...
			return $this->cache_db_multi_get($keys,$life);
		}
	}
	...
	public function cache_db_find_fetch_key($table, $pri, $where = array(), $order = array(), $start = 0, $limit = 0, $life = 0) {
		if($this->cache_conf['enable'] && $this->cache_conf['l2_cache'] === 1 && $life > 0) {
