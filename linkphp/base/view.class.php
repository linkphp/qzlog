...
class view{
	...
	//前台display
	public function index_display($filename = null) {
		$_ENV['_tplname'] = is_null($filename) ? $_GET['control'].'_'.$_GET['action'].'.htm' : $filename;
		extract($this->vars, EXTR_SKIP);
		include $this->index_get_tplfile($_ENV['_tplname']);
	}

	private function index_get_tplfile($filename) {
		$view_dir = F_APP_NAME.($_ENV['_view_diy'] ? '_view_diy' : '_view').'/'; // qzlog_view/
		$php_file = RUNTIME_PATH.$view_dir.$_ENV['_theme'].','.$filename.'.php';// /Users/link/WWW/qzlog/runtime/qzlog_view/default_blog,index.htm.php
		if(!is_file($php_file) || DEBUG) {
			$tpl_file = ROOT_PATH.'theme/'.$_ENV['_theme'].'/'.$filename;

			if(!$tpl_file) {
				throw new Exception('模板文件 '.$_ENV['_theme'].'/'.$filename.' 不存在');
			}

			if(FW($php_file, $this->tpl_parse($tpl_file,1)) === false) {
				throw new Exception("写入模板编译文件 $filename 失败");
			}
		}
		return $php_file;
	...
	private function index_parse_inc($matches) {
	...
	private function tpl_parse($tpl_file,$index=0) {
		...
		//第1步 包含inc模板
		$s = preg_replace_callback('#\{inc\:([\w\.]+)\}#', array($this, $index ? 'index_parse_inc' : 'parse_inc'), $s);
	...
	private function parse_block($matches) {
		...
		$lib_file = core::get_original_file('lp_qzlog_'.$func.'.lib.php', BLOCK_PATH);
		...
		$this->head_arr['lp_qzlog_'.$func] = $lib_str;
		...
		unset($m);
		$func_str = 'lp_qzlog_'.$func.'('.var_export($config_arr, 1).');';

		//-----------定义转换后的首尾代码-----------
		$before = $after = '';
		//公共块移到模板解析代码头部
		if(substr($func, 0, 7) == 'global_') {
			$this->head_arr[$func] = '$qzdata = '.$func_str;
		}else{
		...
		if($_ENV['_view_diy']) {
			$this->lp_qzlog_id++;
			$before .= '<span lp_qzlog_diy="before" lp_qzlog_id="'.$this->lp_qzlog_id.'"></span>';
			$after .= '<span lp_qzlog_diy="after" lp_qzlog_id="'.$this->lp_qzlog_id.'"></span>';
		}
