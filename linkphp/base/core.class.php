...
class core{
	...
	public static function autoload_handler($classname) {
		if(substr($classname, 0, 3) == 'db_') {
			include LINK_PATH.'db/'.$classname.'.class.php';
		}elseif(substr($classname, 0, 6) == 'cache_') {
			include LINK_PATH.'cache/'.$classname.'.class.php';
		}elseif(is_file(LINK_PATH.'ext/'.$classname.'.class.php')) {
			include LINK_PATH.'ext/'.$classname.'.class.php';
		}else{
	...
	public static function init_get() {
		if(!empty($_ENV['_config'][APP_NAME.'_parseurl'])) {
			...
			/*if(isset($_GET['u'])) {
				...
				//匹配例如 http://site_url/user-list.html or http://site_url/user-list
				$rule  = "/\/(.*)-(.*)/";
				$rule_url_suffix  = "/\/(.*)-(.*)".C('url_suffix')."/";
        		preg_match($rule,$path,$result);
        		preg_match($rule_url_suffix,$path,$result_suffix);
        		if (!empty($result_suffix)) {
        			$u = $result_suffix[1].'-'.$result_suffix[2];
        		} else {
        			$u = $result[1].'-'.$result[2];
        		}
			}else{
	...
	public static function clear_code($s) {
		...
		$s = ltrim($s);
		if(substr($s, 0, 29) == 'defined(\'LINK_PATH\') || exit;') $s = substr($s, 29);
